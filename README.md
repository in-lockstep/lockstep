...
| Review conversation as context | runs | what a reviewer said on the pull request — the thread, the verdicts, the notes pinned to a line — reaches the next `/fix` or `/implement` as untrusted context, so replying to a review is running the verb again |
