...
| `GATE-REDACT-1` | P2 | unmet | Enumerate every module writing outside the process; fail on any unwrapped writer. (Default-deny, not an enumerated sink list.) **Missing:** the default-deny sink enumeration. This is the gate the plan called essential after an enumerated list missed five sinks; it was never written. |
