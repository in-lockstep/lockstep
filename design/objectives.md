...
beside it is worth keeping: the first real subject, the Phase 4 merge, has two subject-carrying
runs after it, because the required check's fan-out record carries its lenses as steps and a
step carries no subject. The gate holds on the mechanism, over seeded windows, and its row says
what this repository's own ledger cannot yet show. Every objective is `held`; the ledger's next
