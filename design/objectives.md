...
| `O2` | Onboarding is light | partial | `GATE-PROVISION-1`, `GATE-RECORD-1` | `GATE-PLUGIN-2` | `init` writes a twelve-line `lockstep.py` and a trampoline, which is the achievement N4 records. Two things are not held. The distribution ships no `py.typed`, so the one file a person writes by hand is the one file their type checker cannot see the framework through. And N3 is still open: no gate asserts that a scaffolded repository reaches a green run, and nobody has timed the path from `init` to a first useful review. |
