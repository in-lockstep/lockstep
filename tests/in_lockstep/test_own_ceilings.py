...
def test_gate_sandbox_2_every_binding_that_executes_what_a_model_wrote_is_contained(
    lockstep: Lockstep,
) -> None:
    """GATE-SANDBOX-2, asked of this repository's own module rather than of a scaffold (O10).

    The rule is a property of the BINDING, and the binding is a line a person writes. Validate was
    bound with a bare `Sandbox()` while `CommandValidate` declared `READS_REPO` and nothing else,
    and #410 changed the declaration to the truth: `make lint typecheck` runs recipes over files a
    model can author, and `mypy.ini` is allowed by `ChangeGuard`. Changing the declaration without
    changing the binding turns this repository's own checks off — quietly, in the safe direction,
    which is how it would have stayed unnoticed.

    Derived from the declaration, so a verb bound tomorrow to an adapter that executes is covered
    without this test being edited. `host_fallback` is the same question `staged_refusal` asks, so
    a binding that passes here is one a model-staged run will not be refused for.
    """
    ...
    for verb in (Test, Validate, Build):
        if not lockstep.container.has(verb):
            continue
        adapter = lockstep.container.resolve(verb)
        ...
        # PATH for exactly that case, and the answer names it. The binding is what is under test.
        if why and "on PATH" not in why:
            unconfined.append(f"{verb.__name__}: {why}")

    assert not unconfined, (
        "a binding that executes what a model wrote runs on this host: "
        + "; ".join(unconfined)
        + ". Give it the image and mounts the Test binding has."
    )
