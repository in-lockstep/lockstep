...
def test_review_comment_upserts_a_sticky_pr_comment(repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_a_failed_comment_post_does_not_crash_a_successful_review(
    repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_review_comment_without_a_pr_is_a_note_not_a_failure(
    repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_init_implement_extends_the_module_and_it_loads(repo: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_the_trampoline_is_independent_of_the_repository(
...
def _artifact(tmp_path: Path, *changes: dict[str, Any]) -> Path:
    import json
...
def test_a_workflow_needing_a_human_refuses_when_nobody_asked(repo: Path) -> None:
    ...
    async def go(approval: Approval) -> Outcome[Any]:
        ctx.approval = approval
        call = ActionCall(Implement(ticket=None))

        async def nxt() -> Outcome[Any]:
            return Outcome(status=Status.SUCCEEDED)
...
def _subject(
    guardrails: tuple[tuple[str, str], ...] = (), skills: tuple[tuple[str, str], ...] = ()
) -> EvalSubject:
    ...
    subject: EvalSubject | None = _eval_subject(
        lockstep, kind="review", aspect="security", model_id="anthropic:claude-sonnet-4-6"
    )
...
def _recording_ctx(tmp_path: Path) -> tuple[RunContext, Cassette]:
    from in_lockstep.ai.replay import Cassette
    from in_lockstep.lockstep import Lockstep
...
def test_a_repository_that_binds_its_own_review_adapter_still_records(repo: Path, tmp_path: Path) -> None:
...
def test_the_same_run_says_where_it_put_what_it_kept(repo: Path, tmp_path: Path) -> None:
...
def test_declining_to_record_still_declines(repo: Path, tmp_path: Path) -> None:
...
def test_a_hand_written_adapter_that_bypasses_every_seam_is_named_not_hidden(
    repo: Path, tmp_path: Path
) -> None:
