...
| **N2** | Run the loop on this repository | everyone | **closed in principle, thin in fact.** The framework validates and tests itself in-process, `review` runs the whole path offline, and live model calls have now happened — `/implement` and `/fix` have both run from a comment, and the ledger carries their records. What is still missing is volume: a handful of runs is not a merge rate, and N3 stays open. Since #352 to #354 the required check is `review/all-lenses`, every pull request here gets a sticky comment per lens, and the improve loop measures, judges both arms, proposes and parks on its own draft; the record grows on every merge rather than on request. |
| **N3** | A measured time to first value | leader, small team | open, and the owner's to close: run `init` in a scratch repository, reach a first useful review, and write the minutes here. `init` writes two files and detection now binds the tests of thirteen ecosystems, so the path is shorter than when this row was written; it is still untimed, and an untimed number is not one. |
...
| **N8** | Outcome metrics, not pipeline metrics | leader | **priced, once.** Every outcome carries a `Cost`, and `decided` is a metric dimension so a run that decided nothing cannot read as a success. The first framework-authored change merged here (#343, from `/fix` on #319) cost **$54.04 over ten runs and 3.97 hours of run time** as recorded on `origin/lockstep-history` -- about **$27** in fact, because every AI run's `cost_usd` was doubled until `GATE-COST-7` on 2026-09-08 and the records stay as written: eight attempts blocked or failed before the ninth staged a change that passed, and `fix/propose` opened it for nothing. One change is a price, not a rate; `report --around` can now say what the runs after it looked like, and the rate needs the volume N3 is still waiting on. |
