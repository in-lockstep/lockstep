...
| **N5** | A way to tell a working judge from a broken one | small team | open for rubrics, and no longer open for everything else. `eval harvest` builds cases from real recordings and `eval run` settles their deterministic half against the answer that really came back — so a suite now reports what it decided rather than 27 outstanding forever. A rubric still needs a judge, and still says so. |
...
| **N9** | Transcript retention as a supported decision | security, author | **partly decided.** The expiring tiers are settled and stated where they take effect (`GATE-RECORD-1`): a CI recording lives in the runner's temporary directory and is destroyed with the runner, and the artifact carrying what was harvested from it declares its own retention — 30 days here, 14 in what `init` scaffolds — rather than inheriting a vendor default nobody chose. `doctor` says what is on a laptop and whether git would commit it (DOC168). Still open: the durable tier. A case somebody promotes into the repository is published, not stored, because `git rm` is not deletion and a promoted case stays reachable in every clone.
