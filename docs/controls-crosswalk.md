...
*Replaced* while the mechanism they named had no caller on any live path. That is the same failure
...
| 3 | **Spend ceiling, per day, per agent** | Enforced by the substrate *before a run started*, per agent workflow per day | A rolling 24h per-repository window summed from the ledger's own records, refused pre-run in `Lockstep.context` (`cost.daily_exceeded`), opt-in via `IN_LOCKSTEP_DAILY_LIMIT`; the provider-side organisation limit (`DOC101`) remains the durable backstop | **Replaced, weaker.** The partition is back (per repository, per rolling day, refused before a run starts), but its ledger is this clone's: a runner that never fetched `lockstep-history` sums less than the truth, and two concurrent runs race the read. Weaker where the substrate's store was shared. The SHARED store now exists (`GitLedger(shared=True)`, `GATE-OUT-4`) and is what a park and a resume coordinate through, but the daily window does not read through it: the sum is still `records()` over this checkout, so `history --pull` before the run is what brings a runner's count up to the remote's, and the race between two concurrent readers is unchanged. Summing over the shared store's remote is the remaining step, and this row will say so until it is taken. This row said **Lost** until item 17's timestamps made the window computable. |
| 4 | **Budget is mandatory** | A missing budget was a compile-time **error**; shipping an agent without one was impossible | Startup refusal (`GATE-BUDGET-1`) | **Replaced.** `Lockstep.context()` raises `UndeclaredBudget` when a bound adapter declares `SPENDS_BUDGET` and no ceiling is declared. Narrower than the original in one direction and wider in another: it does not fire for a lifecycle that only runs tests, and it does fire for any adapter that spends rather than only for a compiled agent. `ls` deliberately still works without a budget, so the diagnostic that names what spends survives the refusal that mentions it. A run that cannot spend (`--offline`, `--dry-run`, `apply`) is not exempted: the CLI declares a ceiling of zero, which the gate accepts and the per-run check enforces, so the free path needs no budget argument and a provider that transmitted anyway would be refused at its first projected dollar. |
| 5 | **Wall clock** | `timeout-minutes: 20` on every generated job | `InvokePolicy.deadline_seconds`, re-checked each turn, plus `timeout-minutes` in the scaffolded trampoline | **Replaced.** The CI default without it is 360 minutes, not 20. |
| 6 | **Privilege split** | Safe-outputs: the agent job never held a write token; a separate privileged job applied structured output | Two-job trampoline: `run` holds the provider credential and `contents: read`; `apply` or `publish` holds `contents: write` and no provider credential. The resume trampoline keeps the same split: `lockstep-resume.yml` holds the write token and no provider SDK, and what it runs is decided by the barrier record rather than by the dispatch input | **Replaced.** |
...
- **A replaced chain is the remote's to refuse.** A force-push of freshly fabricated commits
  ...
  "block force pushes" and "restrict deletions" is enough; it needs no reviews, because appends are
...
6. Force-push and deletion protection on `lockstep-history`, so the ledger's append-only claim is
   enforced where a rewrite would otherwise be invisible (see above). Where the store is bound
   `shared=True`, the barrier records live under `refs/lockstep/state/*` and are swapped with
   `--force-with-lease`; a ruleset must not block those updates, or nothing can park or resume.
7. Where the self-feeding loop is wired, the labels it routes and counts on must exist
   ...
   matches and the authorization that trampoline has instead of a gate job, because applying a
   label needs write access where commenting does not. `ai-attempt-N` is where the attempt
...
8. Where a trampoline proposes, Actions must be permitted to open a pull request (`DOC126`).
   ...
   design routes writes through a change request a human reads. The privileged half therefore ends
   in `open_change` and, without that setting, ends there badly: the run does its work, pays for its
