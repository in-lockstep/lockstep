...
| 6 | **Privilege split** | Safe-outputs: the agent job never held a write token; a separate privileged job applied structured output | Two-job trampoline: `run` holds the provider credential and `contents: read`; `apply` or `publish` holds `contents: write` and no provider credential. The resume trampoline keeps the same split: `resume.yml` holds the write token and no provider SDK, and what it runs is decided by the barrier record rather than by the dispatch input | **Replaced.** |
