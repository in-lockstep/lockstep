...
`design/gates.md` now tracks with its `unit only` status. Those rows say so. Consult that file's
status column before citing any row here as evidence.

| 1 | **Egress firewall** | A proxy with an explicit ~55-domain allowlist, enforced outside the agent process | `EgressPolicy` — mandatory on write/execute/network capability, on a restricted repository, **or on any untrusted content in the package**; probe-verified rather than attested | **NOT YET IN FORCE.** `EgressPolicy` is implemented and unit-tested, and `EgressPolicy.check()` has no caller outside those tests — `AiInvoker.run` never consults it, so no run has ever been refused. `GATE-EGRESS-1/2/3` are `unit only` in `design/gates.md`. Until the call site lands, this row is a design, not a control. When it does: the allowlist is computed from resolved provider endpoints, declared MCP endpoints and the telemetry host, so it is tighter than the substrate's bundle — which also means package registries are *not* reachable by default. Decide that before an `EXECUTES_CODE` step needs `pip install`, not during one. |
...
| 4 | **Budget is mandatory** | A missing budget was a compile-time **error**; shipping an agent without one was impossible | Startup refusal (`GATE-BUDGET-1`) | **NOT YET IN FORCE.** `GATE-BUDGET-1` is `unmet`: `Lockstep` defaults to an all-`None` `Budget` and nothing refuses at startup. A compile-time error became neither an error nor a check. |
...
| 9 | **Workflow-file provenance** | *"The workflow that runs is the one on your default branch. A fork cannot modify the workflow that reviews it."* | Trusted config ref — `lockstep.py` and `.in-lockstep/` load from the base ref | **Replaced.** This one was nearly lost: "config is code, discovered at the repository root" silently means the pull request supplies the file defining every control that reviews it. |
...
| 14 | **Protected paths** | `apply_patch` refused writes to CI, guardrail and agent directories | `ChangeGuard`, two tiers, enforced at the write-tool boundary **and** `open_change` **and** `apply` | **Partially in force.** `apply --from-artifact` enforces; the in-loop tool boundary has no tool runner to sit at, and `--apply-inline` does not exist — so one of the three named points is real (`GATE-GUARD-1`, `unmet`). `lockstep.py` now leads tier 1, which is new and necessary: configuration is executable, so an agent that edits it can rebind any adapter and grant itself tools. |
| 15 | **Secret redaction** | Applied to prompts and logs | Privileged sink filter, default-deny over every writer, seeded by `Auth` at mint time | **Replaced, with one claim unverified.** `Redact` works and is tested (`GATE-REDACT-2`), but "default-deny over every writer" rests on `GATE-REDACT-1` — the sink enumeration — which is `unmet`. The sink list is therefore enumerated in practice, which is the state the plan warned about after an enumeration missed five sinks. Seeding from `Auth` rather than the environment is what makes a federated short-lived token redactable at all. |
