...
| Trigger, write verb | `issue_comment` (`/implement`) — on an **issue or a pull request**, since a reviewer asks for the next attempt where they are reading; the comment's number is passed through and `ticket_for` resolves it, never an `if:` expression — plus `issues: labeled` (`ai-generated`) | run-pipeline-with-variables (`LOCKSTEP_ISSUE`), manually or via the trigger API — GitLab CI has no issue-comment trigger; a webhook bridge can supply one |
